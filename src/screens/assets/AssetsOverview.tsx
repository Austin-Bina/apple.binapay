...
function AssetsContent() {
  ...
  // ── Portfolio-level 24h change (weighted average of all assets) ─────────
  const portfolioChange = useMemo(() => {
    const withChange = assets.filter(
      a => a.price_change_24h != null && (a.balance ?? 0) * (a.price_usd ?? 0) > 0
    );
    if (!withChange.length) return null;
    const totalWeight = withChange.reduce(
      (acc, a) => acc + (a.balance ?? 0) * (a.price_usd ?? 0), 0
    );
    const weighted = withChange.reduce(
      (acc, a) =>
        acc + (a.price_change_24h! * ((a.balance ?? 0) * (a.price_usd ?? 0))) / totalWeight,
      0
    );
  ...
  // ── Portfolio sparkline — use USDT history as proxy (largest asset) ─────
  ...
  const handleBuy = async () => {
    ...
    navigate("Main", { screen: "Services", params: { screen: "Convert Crypto" } });
  };
  ...
  const handleDeposit = async () => {
    ...
    navigate("Main", {
      screen: "Home",
      params: { screen: SCREENS.ADD_MONEY, params: { screen: SCREENS.DEPOSIT_CRYPTO } },
    });
  };
    ...
    <View style={[s.root, { paddingTop: insets.top }]}>
      ...
      <View style={s.header}>
        ...
        <View style={s.headerRight}>
          <TouchableOpacity style={s.iconBtn} onPress={() => setBalanceVisible(v => !v)}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
        ...
        >
          ...
          <View style={s.currencyToggle}>
            <TouchableOpacity
              style={[s.toggleBtn, currency === "NGN" && s.toggleBtnActive]}
              onPress={() => setCurrency("NGN")}
            >
              <Text style={[s.toggleText, currency === "NGN" && s.toggleTextActive]}>NGN</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[s.toggleBtn, currency === "USD" && s.toggleBtnActive]}
              onPress={() => setCurrency("USD")}
            >
              <Text style={[s.toggleText, currency === "USD" && s.toggleTextActive]}>USD</Text>
            </TouchableOpacity>
          </View>
          ...
          <Text style={s.cardBalance}>
            {balanceVisible
              ? currency === "NGN"
                ? `₦${formattedBalance(grandTotalNgn, "", 2)}`
                : `$${formattedBalance(grandTotalUsd, "", 2)}`
              : "••••••"}
          </Text>

          {/* Real sparkline + real portfolio change ── */}
          <View style={s.sparkRow}>
            <View style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
              <MaterialCommunityIcons
                name={portfolioPositive ? "trending-up" : "trending-down"}
                size={14}
                color={portfolioPositive ? "#86efac" : "#fca5a5"}
              />
              <Text style={[s.changeText, { color: portfolioPositive ? "#86efac" : "#fca5a5" }]}>
                {portfolioChange != null
                  ? `${portfolioPositive ? "+" : ""}${portfolioChange}% Today`
                  : "— Today"}
              </Text>
            </View>
            <Sparkline
              data={portfolioSparkline}
              color={portfolioPositive ? "rgba(134,239,172,0.8)" : "rgba(252,165,165,0.8)"}
              width={100} height={32}
            />
          </View>
          ...
          <View style={s.cardActions}>
            {[
              ...
              <TouchableOpacity key={label} style={s.cardAction} onPress={action}>
        ...
        <View style={s.listSection}>
          ...
          {visibleAssets.length === 0 ? (
            ...
            visibleAssets.map((item) => {
              ...
              const sparkData  = (item.price_history_24h?.length ?? 0) >= 2
                ...
                <View key={item.id} style={s.assetRow}>
                  ...
                  <View style={s.assetRight}>
                    ...
                    <Text style={s.assetValue}>
                      {balanceVisible
                        ? currency === "NGN"
                          ? `₦${formattedBalance(ngnValue, "", 2)}`
                          : `$${formattedBalance(usdValue, "", 2)}`
                        : "••••"}
                    </Text>

                    {/* Real sparkline + change per asset */}
          ...
          <View style={s.assetRow}>
            ...
            <View style={s.assetRight}>
              <Text style={s.assetBalance}>
                {balanceVisible ? `₦${formattedBalance(nairaBalance, "", 2)}` : "••••"}
              </Text>
              <Text style={s.assetValue}>
                {balanceVisible
                  ? currency === "USD"
                    ? `$${formattedBalance(nairaUsd, "", 2)}`
                    : ""
                  : "••••"}
              </Text>
            </View>
...
const s = StyleSheet.create({
  ...
  header:           { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingHorizontal: 16, paddingTop: 12, paddingBottom: 8, backgroundColor: "#f8f9fb" },
  headerTitle:      { fontSize: 22, fontWeight: "800", color: BRAND },
  headerSub:        { fontSize: 13, color: "#6b7280", marginTop: 2 },
  headerRight:      { flexDirection: "row", gap: 8 },
  iconBtn:          { width: 38, height: 38, borderRadius: 12, backgroundColor: "#EEF3FF", justifyContent: "center", alignItems: "center" },

  card:             { marginHorizontal: 16, marginTop: 8, borderRadius: 20, padding: 20, shadowColor: BRAND, shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.3, shadowRadius: 16, elevation: 8 },
  ...
  hideLabel:        { fontSize: 12, color: "#6b7280" },

  assetRow:         { flexDirection: "row", justifyContent: "space-between", alignItems: "center", backgroundColor: "#fff", borderRadius: 16, padding: 14, marginBottom: 8, borderWidth: 1, borderColor: "#f0f0f0" },
  assetLeft:        { flexDirection: "row", alignItems: "center", gap: 12 },
  assetIcon:        { width: 40, height: 40, borderRadius: 20, backgroundColor: "#f3f4f6" },
  assetName:        { fontSize: 14, fontWeight: "600", color: "#111827" },
  assetSymbol:      { fontSize: 12, color: "#6b7280", marginTop: 2 },
  assetRight:       { alignItems: "flex-end" },
  assetBalance:     { fontSize: 14, fontWeight: "600", color: "#111827" },
  assetValue:       { fontSize: 12, color: "#6b7280", marginTop: 2 },
