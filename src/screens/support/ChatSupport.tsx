...
export default function ChatSupport({ navigation, route }: Props) {
  ...
  // Convert your API messages to FlyerHQ format
  const messages: Message[] = useMemo(() => {
    ...
    return messagesData.map((msg: any) => ({
      id: msg.id.toString(),
      author: {
        id: msg.user_id === user.id ? user.id.toString() : systemUser.id.toString(),
        firstName: msg.user_name,
  ...
  const handleAttachment = async () => {
    ...
    if (!result.canceled) {
      ...
      if (size && findFileSize(size) <= allowedSize) {
        setAttachmentDetails(asset)
      } else {
        showToast({
          message: `The file is ${formattedSize} which exceeds the allowed limit of ${formatBytes(
            allowedSizeInBytes
          )}`,
        })
      }
  ...
  return (
    <Screen style={tw.style(Platform.OS === 'ios' && `pb-4`)}>
      <Appbar.Header style={tw`bg-white`}>
        ...
        <View style={tw`w-full px-2 py-2 flex flex-row gap-3 items-center`}>
          <Avatar.Image size={40} style={tw`bg-white rounded-full`} source={require('@assets/icon.png')} />
          <View>
            <Text style={tw`text-zinc-700 text-lg font-medium`}>BinaPay</Text>
      ...
      {isLoading ? (
        ...
        <Chat
          ...
          onSendPress={(message) => handleAddMessage(message)}
      ...
      {attachmentDetails && (
        <View style={tw`h-12`}>
          <AttachmentPreview
            ...
            onRemoveAttachment={() => setAttachmentDetails(null)}
          />
        </View>
      )}
